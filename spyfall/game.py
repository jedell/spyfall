...
from spyfall.engine import AIEngine, BaseEngine
from spyfall.state import GameState
from spyfall.models import Player, ActionHistory

ai_engine = AIEngine()
player_engine = AIEngine() # TODO replace
...
def ask_question(game_state):
    ...
    if current_player.is_ai:
        # TODO lets do engine.do_action(player, state) in future, this way is dumb
        question, next_player_idx = ai_engine.ask_question(current_player,game_state)
        ...
        question, next_player_idx = player_engine.ask_question(current_player, game_state)

def answer_question(game_state):
    """
    Handles the logic for a player answering a question.
    """
    answering_player = game_state.players[game_state.current_player_idx]
    if answering_player.is_ai:
        answer, next_player_idx = ai_engine.answer(answering_player, game_state, game_state.prev_asker_idx)
...
def initiate_indictment(game_state):
    ...
    If multiple players decide to indict the same player, that player is the suspect
    If multiple players decide to indict different players, we resolve an order in which the
    indictment votes occur. TODO: Need to work out how to resolve this
    """
    initiations = []
    for player in game_state.players:
        if player.is_ai:
            initiations.append(ai_engine.initiate_indictment(player, game_state))
        else:
            # Handle human player initiating an indictment
            initiations.append(player_engine.initiate_indictment(player, game_state))

    return any(initiations)
...
def resolve_indictments(game_state):
    ...
    for indictment_player_idx in game_state.indictments:
        ...
        for player in game_state.players:
            if player.is_ai:
                game_state.votes[player.name] = ai_engine.vote(player, game_state, target)
            else:
                # Handle human player resolving an indictment
                game_state.votes[player.name] = player_engine.vote(player, game_state, target)
...
def game_loop(config):
    ...
    while not game_state.game_over:
        ...
        for _ in range(game_state.rounds):
            ...
            initiated = initiate_indictment(game_state)
            if initiated:
                resolve_indictments(game_state)
